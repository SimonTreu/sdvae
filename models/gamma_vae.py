...
class Decoder(nn.Module):
    def __init__(self, opt, device):
        ...
        # 64x10x1x1
        self.layer1 = self._up_conv(in_channels=self.nz,
                                    out_channels=nf_decoder * 4,
                                    kernel_size=3, stride=1, padding=0)
        # 64x19x3x3
        self.layer2 = self._up_conv(in_channels=nf_decoder * 4,
                                    out_channels=nf_decoder * 3,
                                    kernel_size=4, stride=2, padding=1)
        # 64x19x6x6
        self.layer3 = self._up_conv(in_channels=nf_decoder * 3 + self.use_orog + 4,
    ...
    def forward(self, z, coarse_pr,
        ...
        # layer 1
        hidden_state1 = self.layer1(z)

        # layer 2
        hidden_state2 = self.layer2(hidden_state1)

        # layer 3
        layer3_input = [hidden_state2]
        if self.use_orog:
            upscale6 = Upscale(size=48, scale_factor=8, device=self.device)
            layer3_input.append(upscale6.upscale(orog))
        layer3_input += [coarse_pr, coarse_uas, coarse_vas, coarse_psl]
